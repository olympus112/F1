...
let renderTimeConsistency = (
  ...
  g.append("path")
    ...
    .on("mouseover", function (event) {
      ...
      tooltip.text(driver.name).transition().duration(200).style("opacity", 1);
    })
    ...
    let area = d3
      ...
      .x((d) => {
        if (d < graphData.length) {
          return x(d.raceName);
        } else return x(graphData[graphData.length - 1].raceName);
      })
      .y1((d) => {
        if (d < graphData.length) {
          return y(d.value);
...
let renderPositionsGained = (
  ...
  g.selectAll("g")
    ...
    .selectAll("rect")
    .data((d) =>
      keys.map((key) => ({ raceName: d.raceName, key: key, value: d[key] }))
    )
    .enter()
    .append("rect")
...
export default function Details(props) {
  ...
  React.useEffect(() => {
    graphs[props.graph.id](
      props.data[props.graph.id],
      colors,
      compareData,
      props.driver,
      props.compare
